...
pub async fn udp_listener(
    ...
    loop {
        ...
        if len > 0 {
            ...
            #[cfg(debug_assertions)]
            println!("[UDP_LISTENER] Processing packet type: 0x{:02x}", udp_buf[0]);
            ...
            match udp_buf[0] {
                0x01..=0x0F => {
                    ...
                    if let Ok(packet) = Packet::decode(&udp_buf[1..len]) {
                        #[cfg(debug_assertions)]
                        println!("[UDP_LISTENER] Received protocol packet from {peer_addr}: {packet:?}");
                        Arc::clone(&peer_manager)
                            .handle_proto_packet(Arc::clone(&conf), packet, peer_addr, etx.clone())
                            .await?;
                    } else {
                        #[cfg(debug_assertions)]
                        println!("[UDP_LISTENER] Received invalid protocol packet from {peer_addr}");
                    }
                }
                0x10 => {
                    #[cfg(debug_assertions)]
                    ...
                    if len >= 30 {
                        #[cfg(debug_assertions)]
                        println!("[UDP_LISTENER] Received encrypted packet from {peer_addr} ({} bytes)", len);
                        ...
                        #[cfg(debug_assertions)]
                        println!("[UDP_LISTENER] Dropping encrypted packet too small: {} bytes", len);
                    }
                }
                _ => {
                    #[cfg(debug_assertions)]
                    println!("[UDP_LISTENER] Received unknown packet type 0x{:02x} from {peer_addr}", udp_buf[0]);
                }
            }
        } else {
            #[cfg(debug_assertions)]
...
pub async fn keepalive(remote_addr: SocketAddr, sock: Arc<UdpSocket>) -> crate::Result<()> {
    ...
    loop {
        ...
        #[cfg(debug_assertions)]
        println!("[KEEPALIVE] Encoded packet ({} bytes) for {}", packet_bytes.len(), remote_addr);
        ...
        #[cfg(debug_assertions)]
        println!("[KEEPALIVE] Sleeping for {} seconds before next keepalive", crate::KEEPALIVE_INTERVAL);
...
pub async fn handshake(
    ...
    #[cfg(debug_assertions)]
    println!("[HANDSHAKE] Decoded public key: {}", base64::encode(&pubkey_bytes));
    ...
    loop {
        #[cfg(debug_assertions)]
        println!("[HANDSHAKE] Starting handshake round with {} peers", config.peers.len());
        
        // Send handshake packet to each anchor peer
        for (i, peer) in config.peers.iter().enumerate() {
            if let Some(endpoint) = peer.endpoint {
                #[cfg(debug_assertions)]
                println!("[HANDSHAKE] Sending handshake to peer {}/{}: {}", i + 1, config.peers.len(), endpoint);
                
                let packet_bytes = wire_packet.encode()?;
                if let Err(e) = sock.send_to(&packet_bytes, endpoint).await {
                    #[cfg(debug_assertions)]
                    println!("[HANDSHAKE] Error sending handshake packet to {}: {e}", endpoint);
                ...
                #[cfg(debug_assertions)]
                println!("[HANDSHAKE] Skipping peer {}/{} (no endpoint)", i + 1, config.peers.len());
            }
        }
    ...
    while let Some(event) = update_rx.recv().await {
        ...
        match event {
            ConfigUpdateEvent::PeerConnected { pubkey, endpoint } => {
                #[cfg(debug_assertions)]
                println!("[CONFIG_UPDATER] Processing peer connected event for {}", base64::encode(pubkey));
                ...
                {
                    #[cfg(debug_assertions)]
                    println!("[CONFIG_UPDATER] Creating cipher for peer {} at endpoint {}", base64::encode(pubkey), endpoint);
                    ...
                    #[cfg(debug_assertions)]
                    println!("[CONFIG_UPDATER] No shared secret found for peer {}", base64::encode(pubkey));
                }
            ...
            ConfigUpdateEvent::PeerDisconnected { pubkey } => {
                #[cfg(debug_assertions)]
                println!("[CONFIG_UPDATER] Processing peer disconnected event for {}", base64::encode(pubkey));
                
                // Remove from runtime config
                if let Some(peer) = peer_manager.peer_connections.read().await.get(&pubkey) {
                    if let Some(endpoint) = peer.last_endpoint {
                        #[cfg(debug_assertions)]
                        println!("[CONFIG_UPDATER] Removing cipher for endpoint {} from runtime config", endpoint);
                    ...
                    #[cfg(debug_assertions)]
                    println!("[CONFIG_UPDATER] Peer connection not found for {}", base64::encode(pubkey));
                }
...
async fn update_config_file(
    ...
    #[cfg(debug_assertions)]
    println!("[UPDATE_CONFIG_FILE] Starting config file update for peer {}", base64::encode(pubkey));
    ...
    let mut config: Config = {
        ...
        #[cfg(debug_assertions)]
        println!("[UPDATE_CONFIG_FILE] Config file read successfully ({} bytes)", content.len());
    ...
    #[cfg(debug_assertions)]
    println!("[UPDATE_CONFIG_FILE] Looking for peer {} in config", pubkey_str);
    
    if let Some(peer_config) = config.peers.iter_mut().find(|p| p.pub_key == pubkey_str) {
        #[cfg(debug_assertions)]
        println!("[UPDATE_CONFIG_FILE] Found peer config, updating endpoint from {:?} to {:?}", 
        ...
        #[cfg(debug_assertions)]
        println!("[UPDATE_CONFIG_FILE] Peer {} not found in config", pubkey_str);
    }
    ...
    #[cfg(debug_assertions)]
    println!("[UPDATE_CONFIG_FILE] Writing updated config ({} bytes) to file", updated_content.len());
